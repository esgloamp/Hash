...
def crc32_crap(buf: bytes) -> str:
    '''
    效率过低，还是乖乖用库吧。。。
    '''
    crc_table = (0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419,
    ...
    return hex(crc ^ 0xffffffff)[2:].upper().zfill(8)


def crc32(buf: bytes) -> str:
    crc32 = zlib.crc32(buf)
    return hex(crc32)[2:].upper().zfill(8)


def adler32(buf: bytes) -> str:
    adler32 = zlib.adler32(buf)
    return hex(adler32)[2:].upper().zfill(8)
